...
(function () {
    ...
    ko.utils.setHtml = function(node, html) {
        ...
        if ((html !== null) && (html !== undefined)) {
            ...
            // for example <tr> elements which are not normally alloWeb to exist on their own.
...
ko.expressionRewriting = (function () {
    ...
    var stringDouble = '"(?:[^"\\\\]|\\\\.)*"',
        ...
        // although some of the special characters are alloWeb to start it (all but the colon and comma).
...
ko.exportSymbol('virtualElements.alloWebBindings', ko.virtualElements.alloWebBindings);
...
(function () {
    ...
    function validateThatBindingIsAlloWebForVirtualElements(bindingName) {
        var validator = ko.virtualElements.alloWebBindings[bindingName];
        if (!validator)
    ...
    function applyBindingsToNodeInternal(node, sourceBindings, bindingContext, bindingContextMayDifferFromDomParentElement) {
        ...
        if (bindings) {
            ...
            ko.utils.arrayForEach(orderedBindings, function(bindingKeyAndHandler) {
                ...
                if (node.nodeType === 8) {
                    validateThatBindingIsAlloWebForVirtualElements(bindingKey);
                }
...
ko.virtualElements.alloWebBindings['foreach'] = true;
...
function makeWithIfBinding(bindingKey, isWith, isNot, makeContextCallback) {
    ...
    ko.virtualElements.alloWebBindings[bindingKey] = true;
}
...
ko.virtualElements.alloWebBindings['text'] = true;
        ...
        if (valueHasChanged) {
            ...
            if (valueIsSelectOption) {
                if (newValue !== ko.selectExtensions.readValue(element)) {
                    ...
                    // because you're not alloWeb to have a model value that disagrees with a visible UI selection.
                    ko.dependencyDetection.ignore(ko.utils.triggerEvent, null, [element, "change"]);
                } else {
...
(function () {
    ...
    ko.expressionRewriting.bindingRewriteValidators['template'] = function(bindingValue) {
        ...
        if ((parsedBindingValue.length == 1) && parsedBindingValue[0]['unknown'])
            return null; // It looks like a string literal, not an object literal, so treat it as a named template (which is alloWeb for rewriting)
    ...
    ko.virtualElements.alloWebBindings['template'] = true;
})();
